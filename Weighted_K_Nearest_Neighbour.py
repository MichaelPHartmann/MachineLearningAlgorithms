...
class weightedKNearestNeighbour():
    ...
    def nearest_neighbours(self):
        ...
        for i in range(self.k):
            self.nodes_closest.append(self.node_distances[i])
    ...
    def label_test_node(self):
        ...
        for n in self.nodes_closest:
            ...
            label = node.label
            total += distance
            if node.label in label_frequency.keys():
                 label_frequency[node.label] += distance
            else:
                label_frequency[node.label] = distance
        for f in label_frequency.keys():
            label_frequency[f] = 1 - (label_frequency[f] / total)
        highest_frequency = 0
        top_label = 0
        for l, f in label_frequency.items():
